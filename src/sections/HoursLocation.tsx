...
export function HoursLocation({ business }: HoursLocationProps) {
  ...
  return (
    <section id="horarios" className="py-24 sm:py-32" aria-labelledby="horarios-heading">
      <Container>
        <Reveal>
          <SectionHeading
            eyebrow="Horários & Localização"
            title={<span id="horarios-heading">Quando e onde te esperamos</span>}
            align="center"
            className="mx-auto"
          />
        </Reveal>
        ...
        <div className="mt-14 grid gap-8 lg:grid-cols-2">
          <Reveal className="rounded-3xl border border-border bg-surface p-8">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-display text-2xl uppercase text-cream">Horário de funcionamento</h3>
            </div>
            ...
            >
              <span className={cn('size-2 rounded-full', status.isOpen ? 'bg-emerald-400' : 'bg-cream/40')} />
              {status.label}
            </span>
          </Reveal>

          <Reveal delay={0.1} className="flex flex-col overflow-hidden rounded-3xl border border-border bg-surface">
            <iframe
